...
class MortgageEngine:
    ...
    def simulate_manual_tracks(self, tracks_data: List[Dict],property_value) -> Dict:
        ...
        for i, tr in enumerate(tracks_data):
            ...
            if tr['rate'] == 0:
                init_rate_dic = self.calculator.get_adjusted_rate(con.ANCHOR_TRACK_MAP[tr['rate_type']], capital_allocation, tr['freq'], tr['months'])
                init_ogen,init_tosefet,init_sum_rate = init_rate_dic['ogen'],init_rate_dic['tosefet'],float(init_rate_dic['sum_rate'])
            else:
                init_ogen,init_tosefet,init_sum_rate = 0,0,tr['rate'] 
            
            sch = calculate_schedule(
                    tr['principal'], tr['months'], init_sum_rate, tr['schedule_t'], 
                    tr['rate_type'], tr.get('freq'), con.prime_margin, init_tosefet
                )
    ...
    def check_approval_analysis_new_format(self, file_content: bytes, property_value: float) -> Dict[str, Any]:
        ...
        results["proposed_mix"] = {
            "summary": {
                ...
                "high_month_payments" :high_month_payments,
            },
    ...
    def get_uniform_baskets_analysis(self, principal: float, years: int) -> Dict[str, Any]:
        ...
        def quick_calc(amt, t_type, freq, name): # new function adapted ogen and tocefet
            #try:
                
            init_rate_dic = self.calculator.get_adjusted_rate(con.ANCHOR_TRACK_MAP[t_type], '75%', freq, months)
            init_ogen,init_tosefet,init_sum_rate = init_rate_dic['ogen'],init_rate_dic['tosefet'],float(init_rate_dic['sum_rate'])
            #except: 
            #    init_sum_rate = 4.0
